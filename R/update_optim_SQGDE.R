#' update_optim_SQGDE
#'
#' @description Updates a set of particle traces and weights for further optimization.
#' Useful when convergence criteria is not met in the alotted iterations or when
#' only particle save file is recovered.
#' Uses Stochastic Quasi-Gradient Differential Evolution (SQG-DE;
...
#'
#' @param ObjFun The same ObjFun used in optim_SQGDE. Must have the same length as the former run.
#'
#' @param last_old_particles The last valid iteration of a particle trace container from the output of optim_SQGDE.
#' Requires the same dimensions of the particles and ObjFun as former run.
#'
#' @param last_old_weights The last valid iteration of a weights trace container from the output of optim_SQGDE.
#' Requires the same dimensions of the particles and parameters as former run.
#'
#' @param control_params A list of control parameters for the SQG-DE algorithm as used in optim_SQGDE. The primary required
...
update_optim_SQGDE = function(ObjFun,
                       last_old_particles,
                       last_old_weights,
  ...
  particles[1,,] = last_old_particles
  weights[1,,] = last_old_weights
