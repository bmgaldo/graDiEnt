...
SQG_DE_bin_1_best=function(pmem_index,
                           current_params,
  ...
  parent_indices = sample(c(1:n_particles)[-best_pmem_index],size=2*n_diff,replace=FALSE)
  ...
  if(all(is.finite(grad_approx)) & is.finite(psi) & (psi>0)){
    ...
    params_update[param_indices] = current_params[best_pmem_index,param_indices] -
